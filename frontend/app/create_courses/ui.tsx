...
export default function UI() {
	...
	return (
		<div className="w-full max-w-xl mx-auto h-[90vh] flex flex-col items-center justify-center gap-4">
			...
			<Input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="제목을 입력해주세요." className="bg-[#F6F6F6] py-6 rounded-xs" />
			<div className="space-x-2">
				<Button variant={"outline"} className="px-8 py-6 text-md font-bold">
					이전
				</Button>
				<Button onClick={() => createCourseMutation.mutate()} variant={"default"} className="px-8 py-6 text-md font-bold">
					만들기
				</Button>
			</div>
		</div>
