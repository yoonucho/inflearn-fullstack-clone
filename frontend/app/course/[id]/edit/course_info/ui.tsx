...
export default function EditCourseInfoUI({ course }: { course: Course }) {
	...
	return (
		<Form {...form}>
			<form onSubmit={handleSubmit((data: FormValues) => updateCourseMutation.mutate(data))} className="space-y-8 bg-white p-8 rounded-lg shadow">
				<FormField
					control={control}
					name="title"
					render={({ field }) => (
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<div className="text-xs text-red-500 mb-1">강의소개 상단에 보여집니다. 잠재 수강생들이 매력을 느낄만한 글을 짧게 남겨주세요.</div>
							<FormControl>
								<Textarea {...field} placeholder="ex) 이 강의를 통해 수강생은 컴퓨터 공학의 기초를 다질 수 있을 것으로 예상합니다." required rows={3} />
							</FormControl>
							<FormMessage />
						</FormItem>
					)}
