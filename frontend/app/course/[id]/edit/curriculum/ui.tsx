		...
		<div className="space-y-8">
			...
			{course.sections?.map((section: Section, sectionIdx: number) => (
				<div key={section.id} className="border rounded-lg p-4 bg-white">
					<div className="flex items-center justify-between mb-2">
						...
						<div className="flex items-center gap-2">
							<Button variant="ghost" size="icon" onClick={() => handleDeleteSection(section.id)} className="text-red-500 hover:bg-red-100" aria-label="섹션 삭제">
								<Trash2 size={18} />
							</Button>
						</div>
					</div>
							...
							<div key={lecture.id} className="flex items-center justify-between px-2 py-2 border rounded-md bg-white">
								...
								<div className="flex items-center gap-2">
									<Button
										...
										aria-label="미리보기 토글"
									>
										{lecture.isPreview ? <LockOpen className="text-green-600" size={18} /> : <Lock className="text-gray-400" size={18} />}
									</Button>
									{/* 수정 버튼 추가 */}
									<Button
										variant="ghost"
									...
									<Button variant="ghost" size="icon" onClick={() => handleDeleteLecture(lecture.id)} className="text-red-500 hover:bg-red-100" aria-label="강의 삭제">
										<Trash2 size={18} />
									</Button>
								</div>
							</div>
			...
			<div className="border rounded-lg p-4 bg-gray-50">
				<div className="flex items-center gap-2 mb-2">
					...
					<Input className="w-64" value={addSectionTitle} onChange={e => setAddSectionTitle(e.target.value)} placeholder="섹션 제목을 작성해주세요. (최대 200자)" maxLength={200} />
			...
			<Dialog open={lectureDialogOpen} onOpenChange={setLectureDialogOpen}>
				<DialogContent>
					...
					<Input value={addLectureTitle} onChange={e => setAddLectureTitle(e.target.value)} placeholder="제목을 입력해주세요. (최대 200자)" maxLength={200} />
					<DialogFooter>
