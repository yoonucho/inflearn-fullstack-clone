	...
	return (
		<div className="max-w-xl mx-auto bg-white p-8 rounded-lg mt-10 shadow">
			...
			<form onSubmit={handleSave} className="space-y-6">
					...
					<div {...getRootProps()} className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer ${isDragActive ? "border-primary" : "border-gray-300"}`}>
						...
						<div className="py-4 flex flex-col items-center">
							{isUploading ? (
								...
								<img src={image} alt="프로필 미리보기" className="w-24 h-24 object-cover rounded-full mx-auto mb-2 border" />
							) : (
								<ImageIcon className="w-10 h-10 text-gray-400 mb-2" />
							)}
							<span className="text-sm text-gray-600">{image ? "클릭하여 변경" : isDragActive ? "이미지를 여기에 놓으세요" : "클릭하거나 이미지를 드래그하여 업로드"}</span>
						</div>
					</div>
				...
				{/* 저장 버튼 */}
				<div className="pt-2">
					<Button disabled={updateProfileMutation.isPending} type="submit" className="w-full text-md font-bold" size={"lg"}>
