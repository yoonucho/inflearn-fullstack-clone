...
export default function SiteHeader({ session, profile, categories }: { session: Session | null; profile?: User; categories: CourseCategory[] }) {
		...
		<header className="site-header w-full border-b bg-white">
			...
			<div className="header-top flex items-center justify-between px-8 py-3 gap-4">
				...
				<div className="logo min-w-[120px]">
					<Link href="/">
						<Image src="/images/inflearn_public_logo.png" className="w-28 h-auto" width={120} height={32} alt="inflearn" />
					</Link>
				</div>
				{/* 네비게이션 */}
				...
				<div className="flex-1 flex justify-center">
					<div className="relative flex w-full max-w-xl items-center">
						<Input type="text" placeholder="나의 진짜 성장을 도와줄 실무 강의를 찾아보세요" className="w-full bg-gray-50 border-gray-200 focus-visible:ring-[#1dc078] pr-10" />
						<button type="button" className="absolute right-2 p-1 text-gray-400 hover:text-[#1dc078] transition-colors" tabIndex={-1}>
							<Search size={20} />
						</button>
					</div>
				</div>
				...
				<Link href="/instructor">
					<Button variant="outline" className="font-semibold border-gray-200 hover:border-[#1dc078] hover:text-[#1dc078]">
						지식공유자
					</Button>
				</Link>
				...
				{session ? (
					<Popover>
						...
						<PopoverContent align="end" className="w-56 p-0">
							...
							<button className="w-full text-left px-4 py-3 hover:bg-gray-100 focus:outline-none border-t border-gray-100" onClick={() => signOut()}>
					...
					<Link href="/signin">
						<Button variant="outline" className="font-semibold border-gray-200 hover:border-[#1dc078] hover:text-[#1dc078] ml-2">
							로그인
						</Button>
					</Link>
				)}
